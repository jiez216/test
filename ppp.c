...
void processchap(sessionidt s, tunnelidt t, uint8_t *p, uint16_t l)
{
	...
	sprintf(log_msg, "type:[chap_request] direct:[client->l2tpns]");

	if (l < 4)
	{
		LOG(1, s, t, "Short CHAP %u bytes\n", l);
		STAT(tunnel_rx_errors);
		sprintf(log_msg, "%s username:[%s] error:[short CHAP packet]", log_msg, session[s].user);
	...
	if ((hl = ntohs(*(uint16_t *) (p + 2))) > l)
	{
		...
		STAT(tunnel_rx_errors);
		sprintf(log_msg, "%s username:[%s] error:[CHAP length mismatch]", log_msg, session[s].user);
	    	write_log(log_msg, LOGFILE_TYPE_LOGIN);
	...
	if (session[s].ppp.phase != Authenticate)
	{
		...
		sprintf(log_msg, "%s username:[%s] error:[CHAP ignored in %s phase]", log_msg, session[s].user, ppp_phase(session[s].ppp.phase));
	    	write_log(log_msg, LOGFILE_TYPE_LOGIN);
		return;
	}
	...
	if (!r)
	{
		...
		sprintf(log_msg, "%s username:[%s] error:[Unexpected CHAP message]", log_msg, session[s].user);
	    	write_log(log_msg, LOGFILE_TYPE_LOGIN);
	...
	if (p[1] != radius[r].id)
	{
		...
		STAT(tunnel_rx_errors);
		sprintf(log_msg, "%s username:[%s] error:[Unexpected CHAP response ID]", log_msg, session[s].user);
	    write_log(log_msg, LOGFILE_TYPE_LOGIN);
	...
	if (l < 5 || p[4] != 16)
	{
		...
		STAT(tunnel_rx_errors);
		sprintf(log_msg, "%s username:[%s] error:[Bad CHAP response length]", log_msg, session[s].user);
	    write_log(log_msg, LOGFILE_TYPE_LOGIN);
	...
	if (l < 16 || l - 16 >= sizeof(session[s].user))
	{
		...
		STAT(tunnel_rx_errors);
		sprintf(log_msg, "%s username:[%s] error:[CHAP user too long]", log_msg, session[s].user);
	    write_log(log_msg, LOGFILE_TYPE_LOGIN);
	...
	// Run PRE_AUTH plugins
	{
		...
		LOG(1,s, t, "reachbit: packet.username: %s\n", packet.username);
		char pstr_client_version[30];
