...
#define MAXTBFS		65535		// Maximum token bucket filters. Might need up to 2 * session.
...
typedef struct
{
	...
	// by Laurence
	char check_sessionid_id;
	char check_sessionid_pass[16];
	char check_sessionid_auth[16];
	
}
sessiont;

//add by Laurence
typedef struct t_kick_session
{
	struct t_kick_session * next;
...
typedef struct
{
	...
	in_addr_t   bind_address_secondary;
...
void processudp_with_fd(uint8_t *buf, int len, struct sockaddr_in *addr, int udp_fd);
...
//add by Laurence
#define LOGFILE_TYPE_LOGIN  1
#define LOGFILE_TYPE_KICK    2

#define LOGFILE_FORMAT_KICK  "/usr/local/vpn/l2tpns/etc/l2tpns/l2tpns_kick_%04d%02d%02d.log" 
#define LOGFILE_FORMAT_LOGIN  "/usr/local/vpn/l2tpns/etc/l2tpns/l2tpns_login_%04d%02d%02d.log" 
