...
int udpfd_secondary = -1; //UPD file handle secondary
...
config_descriptt config_values[] = {
	...
	CONFIG("bind_address", bind_address, IPv4),
      CONFIG("bind_address_secondary", bind_address_secondary, IPv4),
...
void _log(int level, sessionidt s, tunnelidt t, const char *format, ...)
{
	...
	if (ringbuffer)
	{
		...
		va_start(ap, format);
		//++modified by tianing 2011-08-11,modified as official advise
...
static void initudp(void)
{
    ...
    if(config->bind_address_secondary != 0)
	{
	    struct sockaddr_in addr_secondary;
		int on_secondary = 1;
		memset(&addr_secondary, 0, sizeof(addr_secondary));
	    addr_secondary.sin_family = AF_INET;
	    addr_secondary.sin_port = htons(L2TPPORT);
	    addr_secondary.sin_addr.s_addr = config->bind_address_secondary;
	    udpfd_secondary = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	    setsockopt(udpfd_secondary, SOL_SOCKET, SO_REUSEADDR, &on_secondary, sizeof(on_secondary));
	    {
		    int flags = fcntl(udpfd_secondary, F_GETFL, 0);
		    fcntl(udpfd_secondary, F_SETFL, flags | O_NONBLOCK);
	    }
	    if (bind(udpfd_secondary, (void *) &addr_secondary, sizeof(addr_secondary)) < 0)
	    {
		    printf("Error in UDP second bind: %s\n", strerror(errno));
			LOG(0, 0, 0, "Error in UDP second bind: %s\n", strerror(errno));
		    exit(1);
	    }
	}
	//end modified
...
// set up TCP fd, communicate with node server
//listen on 127.0.0.1, port 1702?
static void init_tcp(void)
{
	int port_server = 1723;
	...
	if (fd_listen < 0)
	{
   		LOG(0, 0, 0, "create tcp listen socket failed\n");
   		exit(1);
	}
	else
	{
		LOG(0, 0, 0, "create tcp socket on fd: %d success\n", fd_listen);
	}

     	int s = bind(fd_listen, (struct sockaddr *)&addr_server, sizeof(addr_server));
	//check bind result
	if (s < 0)
	{
      		LOG(0, 0, 0, "bind address on fd: %d failed\n", fd_listen);
      		close(fd_listen);
      		exit(1);
	}

      	//set non block 
	if (make_socket_non_blocking(fd_listen) < 0)
	{
		LOG(0, 0, 0, "set non block on fd: %d failed\n", fd_listen);
		exit(1);
	}
	else
	{
		LOG(0, 0, 0, "set non block on fd: %d success!\n", fd_listen);
	}

	//listen
	s = listen(fd_listen, 100);
	//listen error
	if (s < 0)
	{
		LOG(0, 0, 0, "tcp socket listen() on fd: %d failed\n", fd_listen);
		exit(1);
	}
	else
	{
		LOG(0, 0, 0, "tcp server is listening on fd: %d\n", fd_listen);
	}

}


static kick_session_t * insert_kick_session(kick_session_t * head, kick_session_t * p_kick_session)
{
	//empty element
 	if (p_kick_session == NULL)
		return head;
	
	LOG(2, 0, 0, "insert %s into kick list!\n", p_kick_session->user_name);
	...
	kick_session_t * p = head;
	if (p_kick_session->opened >= p->opened)
	{
		p_kick_session->next = p;
		head = p_kick_session;
		return head;
	}
	while(p->next)
	{
		if (p_kick_session->opened < p->next->opened)
		{
			p = p->next;
		}
		else
		{
			p_kick_session->next = p->next;
			p->next = p_kick_session;
			break;
		}
	}
...
static void  free_kick_list(kick_session_t * head)
{
	if (head == NULL)
		return;

	kick_session_t * p = head;
	kick_session_t * q = p->next;
	while(p)
	{
		free(p);
		p = NULL;
	...
	head = NULL;
	LOG(2, 0, 0, "free kick list success.\n");
}


static void dump_kick_list(kick_session_t * head)
{
	
	if (head == NULL)
	{
		LOG(2, 0, 0, "-------------------kick list----------------\n");
		LOG(2, 0, 0, "empty list!\n");
		return;
	}
	...
	while (p)
	{
		LOG(2, 0, 0, "session id: %d, user_name: %s\n", p->session_id, p->user_name);
		p = p->next;	
	}
}

//number_kick, the number of session to kick.
static int kick_session(int number_kick)
{
   	if (config->enable_kick_user == 0)
      	{
		LOG(2, 0, 0, "didn't allow to kick user by node server, check configure file\n");
  		return 0;
	}

	kick_session_t * kick_list = NULL;
	...
	for (s = 1; s <= config->cluster_highest_sessionid ; ++s)
	{
		...
		//find kick element	
		if ((strlen(session[s].user) > 16 && strncmp(session[s].user, "FREE_", 5) == 0)
			||strcmp(session[s].user, "156516143") == 0)
		{
			uint32_t has_opened = abs(now_time-session[s].opened);
		     	if (has_opened < config->kick_user_time_limit*60)
		     	{
				LOG(1, 0, 0, "not allow to kick, opened: %d, kick_user_limit: %d\n", has_opened, config->kick_user_time_limit*60);
				continue;
		     	}

			kick_session_t * p_kick_session = NULL;
			p_kick_session = (kick_session_t*)malloc(sizeof(kick_session_t));
			if (p_kick_session == NULL)
			{
				LOG(0, 0, 0, "malloc failed in kick_session!\n");
				continue;
			}
			memset(p_kick_session, 0, sizeof(kick_session_t));
			p_kick_session->next = NULL;
			p_kick_session->session_id = s;
			p_kick_session->opened = has_opened;
			strcpy(p_kick_session->user_name, session[s].user);
			kick_list = insert_kick_session(kick_list, p_kick_session);
		}

	}

	//dump the to_kick session list.
	dump_kick_list(kick_list);

	//kick number_kick sessions on top of the sorted list
	int count_remain_to_kick = number_kick;
	kick_session_t * p = kick_list;
	while(p)
	{
		if(count_remain_to_kick--<=0)
			break;
		sessionidt s = p->session_id;
		if (session[s].ip && session[s].opened && !session[s].die)
		{
			LOG(2, 0, 0, "Dropping user %s!\n", session[s].user);
                   char log_msg_kick[100];
			memset(log_msg_kick, 0, 100);
			sprintf(log_msg_kick, "kick user: %s ip: %s type: 1", session[s].user,fmtaddr(htonl(tunnel[ session[s].tunnel ].ip), 1));
			write_log(log_msg_kick, LOGFILE_TYPE_KICK);
...
// return: pointer to user name string in heap if sessionid of valid format.
//           remember to free when used.
static char * get_user_name_from_sessionid(char * pstr_sessionid)
{
	if (pstr_sessionid == NULL)
		return NULL;
	if (!*pstr_sessionid)
		return NULL;

      char * pstr_prefix = "XLJS";
	if (strncmp(pstr_sessionid,  pstr_prefix, 4) != 0)
	{
		LOG(0, 0, 0, "bad sessionid format: not start with XLJS\n");
		return NULL;
	}

      	int i_len_sessionid = strlen(pstr_sessionid);
	if (i_len_sessionid < 4)
	{
		LOG(0, 0, 0, "bad sessionid format: length: %d < 4", i_len_sessionid);
		return NULL;
	}

	int i =4;	
	char * pstr_user_name = NULL;
	// Free_adfdfdfdfdfdfddQ
	if(pstr_sessionid[4] == 'F' || pstr_sessionid[4] == 'f')
	{
		if (i_len_sessionid < 25)
		{
			LOG(0, 0, 0, "bad sessionid format: Free user length: %d < 25", i_len_sessionid);
			return NULL;
		}
		pstr_user_name = (char*)malloc(22);
		if (pstr_user_name == NULL)
		{
			return NULL;
		}
		memset(pstr_user_name, 0, 22);
		memcpy(pstr_user_name, pstr_sessionid+4, 21);
		return pstr_user_name;
	}
	// 1989565656
	else if( pstr_sessionid[4] >= '0' && pstr_sessionid[4] <= '9')
	{
		if(i_len_sessionid < 5)
		{
			LOG(0, 0, 0, "bad sessionid format: digital user length: %d < 5", i_len_sessionid);
			return NULL;
		}
		i = 4;
		while(pstr_sessionid[i])
		{
			if(pstr_sessionid[i] < '0' || pstr_sessionid[i] > '9')
			{
				break;
			}
			i++;
		}
		pstr_user_name = (char*)malloc(i-4+1);
		if (pstr_user_name == NULL)
		{
			return NULL;
		}
		memset(pstr_user_name, 0, i-4+1);
		memcpy(pstr_user_name, pstr_sessionid+4, i-4);
		return pstr_user_name;
	}
	else
	{
...
static int check_sessionid(char * pstr_sessionid, sessionidt s)
{
	//assert(pstr_sessionid != NULL && *pstr_sessionid);
	unsigned char pass[16];
	memset(pass, 0, 16);
	MD5_CTX ctx;
	MD5_Init(&ctx);
	MD5_Update(&ctx, &session[s].check_sessionid_id, 1);
	MD5_Update(&ctx, pstr_sessionid, strlen(pstr_sessionid));
	MD5_Update(&ctx, session[s].check_sessionid_auth, 16);
	MD5_Final(pass, &ctx);

	if( memcmp(pass, session[s].check_sessionid_pass, 16) == 0)
	{
		LOG(2, 0, 0, "match the sessionid: success!\n");
		return 0;
	}

    	LOG(2, 0, 0, "match the sessionid: fail!\n");
	//LOG_HEX(0, "id: ", &session[s].check_sessionid_id, 1);
	//LOG_HEX(0, "auth: ", session[s].check_sessionid_auth, 16);
	//LOG_HEX(0, "pass: ", session[s].check_sessionid_auth, 16);
	
      return -1; 
}


//add by Laurence, 2012-07-26
//kick session by sessionid.
//user_name, the user name of session to kick.
static int kick_session_by_sessionid(char * pstr_sessionid)
{
	kick_session_t * kick_list = NULL;
	...
	if (pstr_sessionid == NULL)
	{
		LOG(2, 0, 0, "param pstr_sessionid is NULL\n");
		return -1;
	}

	//get user name from sessionid
	char * pstr_user_name = get_user_name_from_sessionid(pstr_sessionid);
	if(pstr_user_name == NULL)
	{
		LOG(2, 0, 0, "can't parse user_name from pstr_sessionid\n");
		return -1;
	}

	LOG(2, 0, 0, "parse user_name: %s\n", pstr_user_name);
	...
	for (s = 1; s <= config->cluster_highest_sessionid ; ++s)
	{
		//skip unopened session user
		if (!session[s].opened)
			continue;

		//skip walled garden users
		if (session[s].walled_garden)
			continue;		

		//find session by user_name
		if (strcmp(session[s].user, pstr_user_name) == 0)
		{
			kick_session_t * p_kick_session = NULL;
			LOG(2, 0, 0, "session %d match user_name %s!\n", s, session[s].user);
			//match the sessionid
			if (check_sessionid(pstr_sessionid, s) == 0)
			{
				p_kick_session = (kick_session_t*)malloc(sizeof(kick_session_t));
				if (p_kick_session == NULL)
				{
				...
				p_kick_session->next = NULL;
				p_kick_session->session_id = s;
				p_kick_session->opened = abs(now_time-session[s].opened);
				strcpy(p_kick_session->user_name, session[s].user);
				kick_list = insert_kick_session(kick_list, p_kick_session);
	...
	//kick sessions in the sorted list
	kick_session_t * p = kick_list;
	while(p)
	{
		sessionidt s = p->session_id;
		if (session[s].ip && session[s].opened && !session[s].die)
		{
			...
			char log_msg_kick[100];
			memset(log_msg_kick, 0, 100);
			sprintf(log_msg_kick, "kick user: %s ip: %s kick type: 2", session[s].user,fmtaddr(htonl(tunnel[ session[s].tunnel ].ip), 1));
			write_log(log_msg_kick, LOGFILE_TYPE_KICK);
			cli_session_actions[s].action |= CLI_SESS_KILL;
		}
		p = p->next;
...
/*
//add by Laurence, 2012-06-15
//kick session by user name.
//user_name, the user name of session to kick.
static int kick_session(char * user_name)
{
	kick_session_t * kick_list = NULL;
	...
	for (s = 1; s <= config->cluster_highest_sessionid ; ++s)
	{
		...
		//find kick element	
		if (strcmp(session[s].user, user_name) == 0)
		{
			kick_session_t * p_kick_session = NULL;
			p_kick_session = (kick_session_t*)malloc(sizeof(kick_session_t));
			if (p_kick_session == NULL)
			...
			p_kick_session->next = NULL;
			p_kick_session->session_id = s;
			p_kick_session->opened = session[s].opened;
			strcpy(p_kick_session->user_name, session[s].user);
			kick_list = insert_kick_session(kick_list, p_kick_session);
		}
	}
	...
	while(p)
	{
             if (strcmp(session[s].user, "156516143") == 0)
             {
			LOG(0, 0, 0, "can't dropping user %s!\n", session[s].user);
			continue;
		}
		sessionidt s = p->session_id;
		if (session[s].ip && session[s].opened && !session[s].die)
		{
			LOG(0, 0, 0, "Dropping user %s!\n", session[s].user);
			cli_session_actions[s].action |= CLI_SESS_KILL;
		}
		p = p->next;
	}
...
void tunnelsend(uint8_t * buf, uint16_t l, tunnelidt t)
{
	...
	*(uint32_t *) & addr.sin_addr = htonl(tunnel[t].ip);
	...
	if (sendto(tunnel[t].udpfd, buf, l, 0, (void *) &addr, sizeof(addr)) < 0)
	{
		LOG(0, ntohs((*(uint16_t *) (buf + 6))), t, "Error sending data out tunnel: %s (udpfd=%d, buf=%p, len=%d, dest=%s)\n",
				strerror(errno), tunnel[t].udpfd, buf, l, inet_ntoa(addr.sin_addr));
		STAT(tunnel_tx_errors);
		return;
	}
	...
	{                          // control
		...
		{                     // if not a null message
			...
			while (l && !(fatal & 0x80)) // 0x80 = mandatory AVP
			{
				...
				//++modified by tianming 2011-09-15,break if n == 0
				if (n > l || n == 0)
				{
...
// read and process packet on tunnel (UDP)
void processudp_with_fd(uint8_t *buf, int len, struct sockaddr_in *addr, int fd_udp)
{
	printf("    processudp\n");
	uint8_t *chapresponse = NULL;
	uint16_t l = len, t = 0, s = 0, ns = 0, nr = 0;
	...
	{
		LOG(1, s, t, "Received UDP packet with invalid session ID\n");
		printf( "        Received UDP packet with invalid session ID\n");
		STAT(tunnel_rx_errors);
		return;
	}
	if (t >= MAXTUNNEL)
	{
		LOG(1, s, t, "Received UDP packet with invalid tunnel ID\n");
		printf( "        Received UDP packet with invalid tunnel ID\n");
		STAT(tunnel_rx_errors);
		return;
	}
	if (*buf & 0x08)
	...
	{    
        ...
        printf( "Control message (%d bytes): (unacked %d) l-ns %d l-nr %d r-ns %d r-nr %d\n",
			l, tunnel[t].controlc, tunnel[t].ns, tunnel[t].nr, ns, nr);
		// if no tunnel specified, assign one
		if (!t)
		{
			printf("            assign tunnel\n");
			if (!(t = new_tunnel()))
			{
				LOG(1, 0, 0, "No more tunnels\n");
			...
			tunnel[t].port = ntohs(addr->sin_port);
			tunnel[t].window = 4; // default window
			tunnel[t].udpfd = fd_udp;
			printf("                New tunnel from %s:%u ID %d\n",
				fmtaddr(htonl(tunnel[t].ip), 0), tunnel[t].port, t);
			STAT(tunnel_created);
			LOG(1, 0, t, "   New tunnel from %s:%u ID %d\n",
				fmtaddr(htonl(tunnel[t].ip), 0), tunnel[t].port, t);
		...
		if (ns != tunnel[t].nr)
		{
			...
			LOG(1, 0, t, "   Out of sequence tunnel %d, (%d is not the expected %d)\n",
				t, ns, tunnel[t].nr);
            printf("                Out of sequence tunnel %d, (%d is not the expected %d)\n",
				t, ns, tunnel[t].nr);
...
// every second
// when i do this code, i miss u so hardly.
static void regular_cleanups(double period)
{
	...
	for (i = 0; i < s_slice; i++)
	{
		...
		{
			int send = 0;
			LOG(0, s, session[s].tunnel, "Dropping session by node server\n");
...
static int get_int(char * buffer)
{
	assert(buffer != NULL);

	int value = 0;
	value = (value<<0) | buffer[3];
	value = (value<<8) | buffer[2];
	value = (value<<16) | buffer[1];
	value = (value<<24) | buffer[0];
	return value;
}


static int get_short(char * buffer)
{
	assert(buffer != NULL);

	int value = 0;
	value = (value<<0) | buffer[1];
	value = (value<<8) | buffer[0];
	return value;
}


static char * get_string(char * buffer, int length)
{
	if(length < 4)
	{
		LOG(0, 0, 0, "bad length %d < 4", length);
		return NULL;
	}

	int i_index = 0;
	int i_len_string = get_int(buffer);
	i_index += sizeof(int);
	if( i_index + i_len_string > length)
	{
		LOG(0, 0, 0, "bad length %d, length in string: %d", length, i_len_string);
		return NULL;
	}

	char * pstr = (char*)malloc(i_len_string+1);
	memset(pstr, 0, i_len_string+1);
	memcpy(pstr, buffer+4, i_len_string);
	pstr[i_len_string] = '\0';
	return pstr;
...
static void mainloop(void)
{
	...
	struct epoll_event events[BASE_FDS + RADIUS_FDS + EXTRA_FDS + TCP_FDS];
	...
	/* setup our fds to poll for input */
	{
		...
		d[i].type = FD_TYPE_UDP_SECONDARY;
		e.data.ptr = &d[i++];
		if(udpfd_secondary != -1)
		{
			epoll_ctl(epollfd, EPOLL_CTL_ADD, udpfd_secondary, &e);
		}
		...
		epoll_ctl(epollfd, EPOLL_CTL_ADD, daefd, &e);
    		...
    		//add by Laurence.
		static struct event_data event_data_tcp;
		event_data_tcp.type = FD_TYPE_TCP;
		event_data_tcp.fd = fd_listen;
		e.data.ptr = &event_data_tcp;
		epoll_ctl(epollfd, EPOLL_CTL_ADD, fd_listen, &e);
		LOG(0, 0, 0, "add epoll event on fd: %d to node server\n", fd_listen);
	}
	...
	while (!main_quit || still_busy())
	{
		...
		if (n)
		{
			...
			for (c = n, i = 0; i < c; i++)
			{
				...
				//add by Laurence.
				// tcp with node server.
				case FD_TYPE_TCP: 
				{
					LOG(1, 0, 0, "epoll:  TCP on fd: %d\n",d->fd);
					//received client
					//if(fd_listen== events[i].data.fd)
					{
						//accept
                                      	int fd_client = -1;
						alen = sizeof(addr);
						fd_client= accept(fd_listen, (struct sockaddr*)&addr,&alen);
						//accept error
						if (fd_client < 0)
						{
							LOG(0, 0, 0, "accept error: %s\n", strerror(errno));
						...
						{
							LOG(1, 0, 0, "accept a client: %d\n", fd_client);
						}

						//add epoll event
						int s = make_socket_non_blocking(fd_client);
						if(s ==-1)
						{
							LOG(0, 0, 0, "can't make non blocking on fd: %d\n", fd_client);
							break;
						}
						/*
						static struct event_data event_data_client;
						*/
						struct event_data * p_event_data_client = NULL;
						p_event_data_client = (struct event_data*)malloc(sizeof(struct event_data));
						//memset(&event_data_client, 0, sizeof(struct event_data));
						memset(p_event_data_client, 0, sizeof(struct event_data));
						struct epoll_event e;
						e.events = EPOLLIN;
						p_event_data_client->type = FD_TYPE_TCP_CLIENT;
						p_event_data_client->fd = fd_client;
						e.data.ptr = p_event_data_client;
						s = epoll_ctl(epollfd, EPOLL_CTL_ADD, fd_client, &e);
						//epoll add error
						if(s ==-1)
						{
							LOG(0, 0, 0, "add epoll failed on fd: %d\n", fd_client);
							break;
						}
						else
						{
							LOG(1, 0, 0, "add epoll  on fd: %d\n", fd_client);
				...
				{
					LOG(1, 0, 0, "epoll:  TCP client on fd: %d\n", d->fd);
					int closed = 0;
					...
					//received data from client
					{
						...
						int over = 0;
						int closed = 0;
						char buffer_tmp[MAX_NODE_SERVER_BUFFER_SIZE];
						while(1)
						{
							memset(buffer_tmp, 0, MAX_NODE_SERVER_BUFFER_SIZE);
							int count = 0;
							count = read(fd_tmp, buffer_tmp, MAX_NODE_SERVER_BUFFER_SIZE);
							if (count == -1)
							{
								if (errno != EAGAIN)
								{
									LOG(0, 0, 0, "read() error  on client fd: %d\n", fd_tmp);
									closed = 1;
								}
								break;
							}
							...
							//parse receive data
							LOG(1, 0, 0, "received client data on fd: %d  length: %d\n", fd_tmp, count);
							if(count <24)
							{
								closed = 1;
								break;
							}
							int i_protocol_version = get_int(buffer_tmp);
							int i_sequence = get_int(buffer_tmp+4);
							int i_len = get_int(buffer_tmp+8);
							int i_none = get_int(buffer_tmp+12);
							i_none = get_short(buffer_tmp+16);
							 
							short cmd = get_short(buffer_tmp+18);
							LOG(1, 0, 0, "cmd: %d\n", cmd);

							//command from node svr to l2tpns
							switch(cmd)
							{
								//kick free user by number.	
								case 0x15:
								{
									int value = get_int(buffer_tmp+20);
									LOG(1, 0, 0, "kick user number: %d\n", value);
									kick_session(value);
									break;
								}
								//kick user by sessionid.
								case 0x17:
								{
									int i_number_sessionid = get_int(buffer_tmp+20);
									char * p_cur = buffer_tmp+24;
									int i_remain_len = count - 24;
									LOG(1, 0, 0, "number of sessionid: %d\n", i_number_sessionid);
									for(i = 0; i < i_number_sessionid; i++)
									{
										char * pstr_sessionid = get_string(p_cur, i_remain_len);
										if (pstr_sessionid != NULL)
										{	
											LOG(1, 0, 0, "received sessionid: %s\n", pstr_sessionid);
											kick_session_by_sessionid(pstr_sessionid);
											int i_used = sizeof(int) + strlen(pstr_sessionid);
											p_cur += i_used;
											i_remain_len -= i_used;
											free(pstr_sessionid);
											pstr_sessionid = NULL;
										...
										{
											LOG(0, 0, 0, "--error--can't resovle sessionid from node svr!\n");
											break;
										}
									}
								...
								default:
									LOG(0, 0, 0, "--error--unrecognize cmd: %d\n", cmd);
									break;
							}
							...
							int value_ret = 200;
							char * p = buffer_send;
    							*(p+1) = (char)((cmd_ret) & 0xff);
    							*p = (char)((cmd_ret)>>8 & 0xff);
    							p = p + 2;
    							*(p+3) = (char)((value_ret) & 0xff);
							*(p+2) = (char)((value_ret>>8) & 0xff);
							*(p+1) = (char)((value_ret>>16) & 0xff);
    							*p = (char)((value_ret>>24) & 0xff);
							write(fd_tmp, buffer_send, 6);*/
							
							
							/*
							char * p_strcmd = (char*)malloc(value);
							if (p_strcmd == NULL)
							{
								LOG(0, 0, 0, "malloc failed!\n");
								closed = 1;
								break;
							}
							memset(p_strcmd, 0, value);
							memcpy(p_strcmd, buffer_tmp+3, value);
							LOG(0, 0, 0, "cmd value: %s\n", p_strcmd);*/
						...
						if (closed == 1)
						{
							LOG(1, 0, 0, "close tcp client on fd: %d \n", fd_tmp);
							close(fd_tmp);
							if(d != NULL)
								free(d);
							d = NULL; 	
			...
			for (c = 0; n && c < config->multi_read_count; c++)
			{
				...
				if (udp_secondary_ready)
				{
					alen = sizeof(addr);
					if ((s = recvfrom(udpfd_secondary, buf, sizeof(buf), 0, (void *) &addr, &alen)) > 0)
					{
						//processudp(buf, s, &addr);
						processudp_with_fd(buf, s, &addr, udpfd_secondary);
						udp_pkts++;
					}
					else
					{
						udp_secondary_ready = 0;
						n--;
					}
				}
...
//param: pstr_logfile_format  :   e.g. "./log/l2tp_login_%04d%02d%02d.log"
//       pstr_logfile_name    :   return the file name.   e.g. "./log/l2tp_login_20120404.log"                               
void get_logfile_name(char * pstr_logfile_format, char * pstr_logfile_name)
{
    time_t iCurrSec = time(NULL);
    struct tm tmstu;
    localtime_r(&iCurrSec, &tmstu);
    sprintf(pstr_logfile_name, 
        pstr_logfile_format,
        tmstu.tm_year + 1900, 
        tmstu.tm_mon + 1, 
        tmstu.tm_mday);
...
//write a log msg into logfile.
//param: pstr_log_msg,  msg to write(with no pid).
void _write_log(char* pstr_logfile_format, 
    char* pstr_log_msg, 
    int i_default_log_buffer_len, 
    int i_logfile_name_buffer_len)
{
    //get process id
    pid_t pid = getpid();
    char* pstr_log_msg_with_time_pid = (char*)malloc(i_default_log_buffer_len);
    if (NULL == pstr_log_msg_with_time_pid)
    {
        return;
    }
    //construct log msg with pid
    memset(pstr_log_msg_with_time_pid, 0, i_default_log_buffer_len);
	time_t t_now = time(NULL); 
    strftime(pstr_log_msg_with_time_pid, i_default_log_buffer_len, "%F %X", localtime(&t_now));	
    sprintf(pstr_log_msg_with_time_pid, "%s pid:[%d] %s\n", pstr_log_msg_with_time_pid, pid, pstr_log_msg);
 
    //get log file name.
    char* pstr_logfile_name = (char*)malloc(i_logfile_name_buffer_len);
    if (NULL == pstr_logfile_name )
    {
        free(pstr_log_msg_with_time_pid);
        return;
    }
    get_logfile_name(pstr_logfile_format, pstr_logfile_name);
    //standard c i/o library
    //write msg into log file.
    FILE* fd = fopen(pstr_logfile_name, "a");
    if (NULL != fd)
    {
        fwrite(pstr_log_msg_with_time_pid, strlen(pstr_log_msg_with_time_pid), 1, fd);
        fclose(fd);
    }

    //free string buffer
    free(pstr_log_msg_with_time_pid);
    free(pstr_logfile_name);
}


//implement this wrapper function
// i_logfile_type, may be a lot log files. differ them with type.
void write_log(char* pstr_log_msg, int i_logfile_type)
{
    if (pstr_log_msg == NULL)
        return;

    switch(i_logfile_type)
    {
        case LOGFILE_TYPE_LOGIN:
        {
            _write_log(LOGFILE_FORMAT_LOGIN, pstr_log_msg, 1024, 1024); 
            break;
        }   

	  case LOGFILE_TYPE_KICK:
        {
            _write_log(LOGFILE_FORMAT_KICK, pstr_log_msg, 1024, 1024); 
            break;
        }   
...
int get_version_from_client_user_version(char * pstr_client_user_version)
{
    if (pstr_client_user_version == NULL)
    {
        return -1;
    }

    LOG(5, 0, 0, "config->unlimit_user_name:%s!\n", config->unlimit_user_name);


    if (strcmp(pstr_client_user_version, config->unlimit_user_name) == 0)
        return -3;

    int i_current_position = 0;
    if(strncmp(pstr_client_user_version, "FREE_", 5) == 0)
    {
        pstr_client_user_version = pstr_client_user_version+5;
        i_current_position = 5;
    }
    while(*pstr_client_user_version)
    {
        if(*pstr_client_user_version == '_')
        {
            return i_current_position;
        }
        pstr_client_user_version++;
        i_current_position++;
    }
...
int get_version_number(char * pstr_version)
{
    ...
    if (i_last_dot == -1)
       return -1;
    char pstr_version_number[10];
    memset(pstr_version_number, 0, 10);
    strncpy(pstr_version_number, pstr_version+i_last_dot+1, 9);
    i_version_number = atoi(pstr_version_number);
