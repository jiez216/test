...
void radiussend(uint16_t r, uint8_t state)
{
	...
	sprintf(log_msg, "type:[login_request] direct:[l2tpns->l2tp_login]");
	...
	if (radius[r].try > config->numradiusservers * 2)
	{
		if (s)
		{
			if (state == RADIUSAUTH)
			{
                sprintf(log_msg, "%s username:[%s] try:[%d] error:[RADIUS timeout]", log_msg, session[s].user, radius[r].try-1);
	...
	if (state == RADIUSAUTH)
	{
		if (radius[r].chap)
		{
			...
			*p = 60;           // CHAP Challenge
			...
			//add by Laurence
			session[s].check_sessionid_id = radius[r].id;
			memcpy(session[s].check_sessionid_pass, radius[r].pass, 16);
			memcpy(session[s].check_sessionid_auth, radius[r].auth, 16);
	...
	*(uint32_t *) & addr.sin_addr = config->radiusserver[(radius[r].try - 1) % config->numradiusservers];
	...
	sprintf(log_msg, "%s username:[%s] try:[%d]", log_msg, session[s].user, radius[r].try-1);
	write_log(log_msg, LOGFILE_TYPE_LOGIN);
...
void processrad(uint8_t *buf, int len, char socket_index)
{
	...
	char log_msg[1024];
	memset(log_msg, 0, 1024);
	sprintf(log_msg, "type:[login_response] direct:[l2tp_login->l2tpns]");

	LOG_HEX(5, "RADIUS Response", buf, len);
	if (len < 20 || len < ntohs(*(uint16_t *) (buf + 2)))
	{
		LOG(1, 0, 0, "Duff RADIUS response length %d\n", len);
		sprintf(log_msg, "%s error:[Duff RADIUS response length]", log_msg);
		write_log(log_msg, LOGFILE_TYPE_LOGIN);
		return ;
	}
	...
	if (!s && radius[r].state != RADIUSSTOP)
	{
		...
		sprintf(log_msg, "%s username:[%s] error:[Unexpected RADIUS response]", log_msg, session[s].user);
		write_log(log_msg, LOGFILE_TYPE_LOGIN);
		return;
	}
	...
	do {
		if (memcmp(hash, buf + 4, 16))
		{
			...
			sprintf(log_msg, "%s username:[%s] error:[Incorrect auth on RADIUS response]", log_msg, session[s].user);
		    write_log(log_msg, LOGFILE_TYPE_LOGIN);
		...
		if (radius[r].state == RADIUSAUTH)
		{
			...
			if (radius[r].chap)
			{
				...
				*(uint16_t *) (p + 2) = ntohs(4); // no message
				...
				sprintf(log_msg, "%s username:[%s] result:[%s]", log_msg, session[s].user, (r_code == AccessAccept) ? "allowed" : "denied");
				write_log(log_msg, LOGFILE_TYPE_LOGIN);

				LOG(3, s, session[s].tunnel, "   CHAP User %s authentication %s.\n", session[s].user,
						(r_code == AccessAccept) ? "allowed" : "denied");

			    sprintf(log_msg, "type:[chap_response] direct:[l2tpns->client] username:[%s] result:[%s]", session[s].user, (r_code == AccessAccept) ? "allowed" : "denied");
			    write_log(log_msg, LOGFILE_TYPE_LOGIN);
